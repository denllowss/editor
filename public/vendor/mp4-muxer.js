...
var Mp4Muxer = (() => {
  ...
  var colr = (track) => {
    const cs = track.info.decoderConfig?.colorSpace;
    if (!cs) return null;
    return box("colr", [
      ascii("nclx"),
      u16(COLOR_PRIMARIES_MAP[cs.primaries] ?? 1),
      u16(TRANSFER_CHARACTERISTICS_MAP[cs.transfer] ?? 1),
      u16(MATRIX_COEFFICIENTS_MAP[cs.matrix] ?? 1),
      u8((cs.fullRange ? 1 : 0) << 7)
    ]);
  };
  var avcC = (track) => {
    const desc = track.info.decoderConfig?.description;
    return desc ? box("avcC", [
      ...new Uint8Array(desc)
    ]) : null;
  };
  var hvcC = (track) => {
    const desc = track.info.decoderConfig?.description;
    return desc ? box("hvcC", [
      ...new Uint8Array(desc)
    ]) : null;
  };
  var vpcC = (track) => {
  ...
  var Muxer = class {
    ...
    addVideoChunk(sample, meta, timestamp, compositionTimeOffset) {
      ...
      this.addVideoChunkRaw(
        ...
        (Number.isFinite(sample.duration) && sample.duration >= 0) ? sample.duration : 0,
        meta,
